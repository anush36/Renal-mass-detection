...
def unet_model(IMG_SIZE, DROPOUT):
    ...
    def attention_block(up_tensor, skip_tensor, num_filters):
        up_tensor = layers.Conv2D(num_filters // 2, (1, 1), padding='same')(up_tensor)
        gating_tensor = layers.Conv2D(num_filters // 2, (1, 1), padding='same')(skip_tensor)
        gating_tensor = layers.Add()([up_tensor, gating_tensor])
        gating_tensor = layers.Activation('relu')(gating_tensor)
        gating_tensor = layers.Conv2D(1, (1, 1), padding='same')(gating_tensor)
        gating_tensor = layers.Activation('sigmoid')(gating_tensor)
        skip_tensor = layers.Multiply()([skip_tensor, gating_tensor])
        return skip_tensor
    ...
    center = conv_block(encoder4_pool, 512) #512
    # 16

    #decoder5 = decoder_block(center, encoder5, 512) #256

    decoder4 = decoder_block(center, encoder4, 256) #256
